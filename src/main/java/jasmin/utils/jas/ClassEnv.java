...
public class ClassEnv implements RuntimeConstants {
    ...
    public Integer getMethodIdxByName(String clazzName, String methName, String signature) {
        Integer index = 0;
        AsciiCP nameCP = new AsciiCP(methName);
        AsciiCP sigCP = new AsciiCP(signature);
        ClassCP clazzCP = new ClassCP(clazzName);
        Enumeration elements = cpe.elements();
        while (elements.hasMoreElements()) {
            Object element = elements.nextElement();
            if (element instanceof MethodCP) {
                MethodCP methCP = (MethodCP) element;
                NameTypeCP ntCP = methCP.nt();
                ClassCP theClazz = methCP.clazz();
                AsciiCP theName = ntCP.name();
                AsciiCP theSignature = ntCP.sig();
                if (theName.equals(nameCP) &&
                        theSignature.equals(sigCP) &&
                theClazz.equals(clazzCP)) {
                    return index;
                }
            }
            index++;
